...
import ChartCard from "@/components/dashboardComponents/admin/analytics/ChartCard";
import {
  ...
  useGetTrendsQuery,
  useGetEventsQuery,
} from "@/api/services/analyticsApi copy";
...
export default function AdminAnalyticsPage() {
  ...
  const { data: eventsData, isLoading: loadingEvents } = useGetEventsQuery({
    limit: 100,
  });
  ...
  const paymentCounts = useMemo(() => {
    const failed =
      summaryData?.find((s) => s._id === "payment.failed")?.count ?? 0;
    const success =
      summaryData?.find((s) => s._id === "payment.success")?.count ?? 0;
    return [
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Realtime operational and business metrics
              </p>
            </div>
            <div className="flex items-center gap-3">
              <Link href="/admin/dashboard">
                {/* <a className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 hover:bg-gray-50">
                  Back to Admin
                </a> */}
                 <button className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-900 hover:bg-gray-50">
                  Back to Admin
                </button>
              </Link>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <StatCard
            ...
            sub={`${kpi.transactions} transactions`}
            icon={DollarSign}
          />
          <StatCard
            title="Orders Created"
            value={kpi.ordersCreated}
            sub={`Placed: ${kpi.ordersPlaced}`}
            icon={ShoppingCart}
          />
          <StatCard
            title="New Users"
            value={kpi.newUsers}
            sub={`Active logins: ${kpi.activeLogins}`}
            icon={Users}
          />
          <StatCard
            title="Conversion Rate"
            value={kpi.funnel.conversion_rate ?? "0%"}
            sub={`${kpi.funnel.orders_placed ?? 0} orders`}
            icon={AlertTriangle}
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="lg:col-span-2">
            <ChartCard
              title="Daily Trends"
              subtitle="Orders / Payments / Signups (per day)"
            >
              <div className="h-64">
                {trendSeries.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={trendSeries}>
                      ...
                      <Line
                        type="monotone"
                        dataKey="order.placed"
                        name="Orders Placed"
                        stroke={COLORS.blue}
                      />
                      <Line
                        type="monotone"
                        dataKey="payment.success"
                        name="Payments"
                        stroke={COLORS.green}
                      />
                      <Line
          ...
          >
            <div className="space-y-3">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Conversion Rate: {kpi.funnel.conversion_rate ?? "0%"}
              </p>
            </div>
          </ChartCard>
        </div>
                  ...
                  <tbody>
                    {eventsData && eventsData.length ? (
                      eventsData.map((e) => {
                        ...
                        const entity =
                          e.payload?.orderId ?? e.payload?.userId ?? "-";
                        return (
                          <tr key={e._id} className="border-t">
                            ...
                            <td className="px-3 py-2 text-gray-600 truncate max-w-[220px]">
                              {entity}
                            </td>
